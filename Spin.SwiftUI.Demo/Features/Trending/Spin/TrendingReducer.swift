...
extension Trending {
    static func reducer(state: Trending.State, event: Trending.Event) -> Trending.State {
        ...
        case (_, .succeedLoad(let viewItems, let currentPage, let previousPage, let nextPage, let totalPage)):
            return .loaded(viewItems: viewItems, currentPage: currentPage, previousPage: previousPage, nextPage: nextPage, totalPage: totalPage)
        default:
            return state
        }
    }
