...
extension Peoples {
    enum Entity {
        ...
        static func search(baseUrl: String, networkService: NetworkService, query: String) -> Single<[People]> {
            let route = Route<ListEndpoint<People>>(baseUrl: baseUrl, endpoint: ListEndpoint<People>(path: PeoplesPath.peopleSearch(query: query)))
        ...
        static func loadDetail(baseUrl: String, networkService: NetworkService, id: String) -> Single<People> {
            let route = Route<EntityEndpoint<People>>(baseUrl: baseUrl, endpoint: EntityEndpoint<People>(path: PeoplesPath.people(id: id)))
